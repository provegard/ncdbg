...
class RemoteObjectConverterTest extends UnitTest with Inside {
  ...
  "RemoteObjectConverter should" - {
    ...
    "convert SimpleValue with an integer-number to a number-based RemoteObject" in {
      converter.toRemoteObject(SimpleValue(42)) should be (RemoteObject.forNumber(42.0d))
    }
    ...
    "convert ErrorValue to a RemoteObject with error details" in {
      ...
      val ev = ErrorValue(data, ObjectId("obj-1"))
    ...
    "convert DateNode to a RemoteObject with a string representation of the date" in {
      ...
      converter.toRemoteObject(date).description should be (stringRep)
    }
  }
}
